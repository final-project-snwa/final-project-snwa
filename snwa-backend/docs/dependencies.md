...
### Selenium(미사용, 추후 삭제 예정)

- 목적: 웹 브라우저 자동화 및 동적 웹 콘텐츠 크롤링을 위한 프레임워크
- 사용 위치: 자바스크립트 렌더링이 필요한 사이트의 데이터 수집, 자동화된 UI 테스트, 멤버십 결제 흐름 테스트 자동화
- 비고: 실제 브라우저(Chrome, Firefox 등)를 제어하므로 Jsoup으로 읽을 수 없는 동적 데이터를 처리할 수 있으나, 리소스 소모가 큼
---

### Jsoup

- 목적: HTML 파싱 및 데이터 추출을 위한 Java 라이브러리
- 사용 위치: 정적 페이지 크롤링, HTML 문서 구조 분석, 외부 상품 정보/리뷰 데이터 스크래핑
- 비고: HTTP 연결부터 DOM 요소 추출까지 직관적인 API를 제공하며, 속도가 빠르고 가벼움

---

### spring-boot-starter-actuator

- 목적: 애플리케이션 상태 모니터링 및 관리 기능 제공
- 사용 위치: 운영 및 관리 (Health check, Metrics 등)
- 비고: /actuator 엔드포인트를 통해 다양한 지표 노출

---

### micrometer-registry-prometheus

- 목적: Actuator 메트릭을 Prometheus 수집 포맷으로 변환
- 사용 위치: 모니터링 시스템 연동 설청
- 비고: Grafana 대시보드 시각화를 위한 데이터 소스로 사용됨
