...
public class ArticleService {
        ...
        public ArticleDetailResponseDto createArticle(User user, ArticleCreateRequestDto request) {
                ...
                return ArticleDetailResponseDto.from(saved);
        }
        ...
        public Page<ArticleListResponseDto> getArticleList(Long categoryId, String publisherName, Pageable pageable,
                ...
                // 번역 데이터 조회 (KO)
                List<Long> articleIds = articles.getContent().stream().map(Article::getId).toList();
                java.util.Map<Long, ArticleTranslation> translationMap = articleTranslationRepository
                                .findAllByArticleIdInAndLanguage(articleIds, "KO")
                                .stream()
                                .collect(Collectors.toMap(t -> t.getArticle().getId(), t -> t));

                return articles.map(a -> ArticleListResponseDto.from(a, translationMap.get(a.getId()),
        ...
        public ArticleDetailResponseDto getArticleDetail(Long id, User user, boolean recordView) {
                ...
                boolean hasUsedCoin = user != null
                                && (user.getRole() == UserRole.ADMIN
                                                || coinTransactionService.hasUsedCoinForArticle(user.getId(), id));
                ...
                List<String> purchasedTranslationLanguages = user != null
                                ? translationAccessLogRepository.findDistinctLanguagesByUserIdAndArticleId(user.getId(),
                                                id)
                                : List.of();

                // 번역 데이터 조회 (KO)
                ArticleTranslation translation = articleTranslationRepository
                                .findByArticleIdAndLanguage(id, "KO").orElse(null);

                return ArticleDetailResponseDto.from(article, translation, isBookmarked, displayClickCount,
                                reactionCounts, hasUsedCoin, purchasedTranslationLanguages);
        }
        ...
        public Page<ArticleListResponseDto> searchArticles(String keyword, Pageable pageable, User user) {
                // 번역 데이터 검색 (KO)
                Page<ArticleTranslation> translations = articleTranslationRepository
                                .searchByKeyword(keyword, "KO", pageable);

                Set<Long> bookmarkedIds = bookmarkService.getBookmarkedArticleIds(user,
                                translations.getContent().stream().map(t -> t.getArticle().getId()).toList());

                return translations.map(t -> ArticleListResponseDto.from(t.getArticle(), t,
                                bookmarkedIds.contains(t.getArticle().getId())));
        }
        ...
        public Page<ArticleListResponseDto> searchByTitle(String keyword, Pageable pageable, User user) {
                // 번역 데이터 검색 (KO)
                Page<ArticleTranslation> translations = articleTranslationRepository
                                .searchByTitle(keyword, "KO", pageable);
        ...
        public Page<ArticleListResponseDto> searchByContent(String keyword, Pageable pageable, User user) {
                // 번역 데이터 검색 (KO)
                Page<ArticleTranslation> translations = articleTranslationRepository
                                .searchByContent(keyword, "KO", pageable);
        ...
        public List<ArticleListResponseDto> getRelatedArticles(Long articleId, User user) {
                ...
                List<Long> articleIds = relatedArticles.stream().map(Article::getId).toList();
                java.util.Map<Long, ArticleTranslation> translationMap = articleTranslationRepository
                                .findAllByArticleIdInAndLanguage(articleIds, "KO")
                                .stream()
                                .collect(Collectors.toMap(t -> t.getArticle().getId(), t -> t));
                ...
                return relatedArticles.stream()
                                .map(a -> ArticleListResponseDto.from(a, translationMap.get(a.getId()),
                                                bookmarkedIds.contains(a.getId())))
